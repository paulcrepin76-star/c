...
**Allow packs**, and replaces the long MangaDex alias list with one short
English `!!` name (`Shangri-La Frontier`, `Bleach`, …) so Nyaa is queried
once. It also adds `erotica` to MangaDex ratings so Add Series search can
see MPD Psycho. Then Add Series search and Interactive Search can see all
five. Have counts stay low until files exist: Bleach and One Piece on disk
are volumes, Naruto has no files yet, Shangri-La has v21 only.
